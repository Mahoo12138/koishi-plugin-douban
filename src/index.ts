...
export const Config = Schema.object({
  searchItemCount: Schema.number()
    .default(3)
    .max(5)
    .min(2)
    .description("展示的搜索结果数量"),
...
export function apply(ctx: Context, config: TemplateOptions) {
  ...
  ctx
    ...
    .action(async ({ session, options }, keyword) => {
      ...
      const count = config.searchItemCount;
      let index = 0;
      if (data.length > 1) {
        const output = [
          session.text("douban.has-multi-result", [keyword, count]),
        ];
        ...
        while (isError) {
          ...
          index = +answer - 1;
          if (--times < 0) return session.text("douban.incorrect-input-too-much");
          // 输入非法内容
          if (!isInteger(index) || index < 0 || index >= count) {
             await session.send(
              session.text("douban.incorrect-index")
            );
          } else {
            isError = false;
          }
      ...
      // 获取详细数据
      const item = await ctx.http.get<string>(url + data[index].id, {
        headers,
      });
      const info = parseRenderData(item);

      let templateCate = "/assets/original.art",
        size = [750, 325];
      ...
      const pic = await page.screenshot({
        clip: { x: 8, y: 8, height: size[1], width: size[0] },
      });
